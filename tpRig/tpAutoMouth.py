...
class mouth_system():
    def __init__(self):
        ...
        if cmds.objExists(self.name + "_BuildPack"):
            ...
            self.buildpack = create_build_pack(self.attr, self.name)
        ...
        if self.executed:
            for item in self.sys_data[0]: 
                try:
                    ...
                    print('Failed to Delete {}, {}'.format(self.sys_data[0][item], self.sys_data[1][item]))
                    pass

            cmds.delete(self.sys_data[2])
        else:
            cmds.warning("System not yet executed")
    ...
    def load_data(self):
        ...
        self.sup_edge_list = cmds.getAttr(mouth_build_pack + ".sup_edge").split(",")
        self.inf_edge_list = cmds.getAttr(mouth_build_pack + ".inf_edge").split(",")
...
def build_corner(sup_ctrl, inf_ctrl, sup_remoteCtrl, inf_remoteCtrl, name):
    ...
    for attr in ".t,.rotate,.s".split(','):
        cmds.connectAttr(corner_ctrl['control'] + attr, corner_remoteCtrl_loc + attr, f=1)

    # Reduce locator scale
    for dimension in ["X", "Y", "Z"]:
        cmds.setAttr("{}.localScale{}".format(corner_remoteCtrl_loc, dimension), 0.4)
...
def locatorOnCVs(path_crv, scale=1.0):
...
def create_build_pack(attr_dict, name):
